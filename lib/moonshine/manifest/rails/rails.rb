...
module Moonshine::Manifest::Rails::Rails
    ...
    if using_bundler?
      ...
      sandbox_environment do
        ...
        bundler_dependencies.each do |dependency|
          rake_present = true if dependency.name.to_sym == :rake
        ...
        unless rake_present
          raise Bundler::GemNotFound, "Please add rake to your gemfile"
        end
      end
      
      bundle_install_without_groups = configuration[:bundler] && configuration[:bundler][:install_without_groups] || "development test"
      exec 'bundle install',
        :command => "bundle install --binstubs --deployment --path #{configuration[:deploy_to]}/shared/bundle --without #{bundle_install_without_groups}",
        :cwd => rails_root,
  ...
  #
  # If bundler is being used, we run the version of rake generated by
  # bundler's --binstubs option.
  def rake(name, options = {})
    ...
    if using_bundler?
      args[:command] = rails_root.to_s + '/bin/' + args[:command]
    end
    exec("rake #{name}", args)
  end

  def gemfile_path
    rails_root.join('Gemfile')
  end
